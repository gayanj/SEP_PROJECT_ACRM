...
namespace ACRM.CPU
{
    class ProcessMethod
    {
        ...
        public static String getUsageForProcess(int pid, ManagementScope connectionScope)
        {
            SelectQuery systemMonitorQuery = new SelectQuery("SELECT PercentProcessorTime FROM win32_PerfFormattedData_PerfProc_Process Where IDProcess = '" + pid + "'");
            ManagementObjectSearcher searchProcedure = new ManagementObjectSearcher(connectionScope, systemMonitorQuery);
            foreach (ManagementObject item in searchProcedure.Get())
            {
                return item["PercentProcessorTime"].ToString();
            }
            return null;
        }
    }
